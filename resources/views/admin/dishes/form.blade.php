...
<section>
    ...
    {{-- * CREATE/EDIT FORM --}}
    <form enctype="multipart/form-data" {{-- Distinguish if the user needs to create or to edit --}} action="{{ empty($dish->id) ? route('admin.dishes.store') : route('admin.dishes.update', $dish) }}" method="POST">
        @if (!empty($dish->id))
        @method('PATCH')
        @endif
        @csrf
            ...
            <div class="col-sm-12 col-md-6">
                ...
                <div class="card p-2">
                    ...
                    <div>
                        ...
                        <input class="form-control @error('name') is-invalid @enderror" id="name" name="name" type="text" max="150" required value="{{ empty($dish->id) ? '' : old('name') ?? $dish->name }}" />
                        @error('name')
                    ...
                    <div class="mt-2">
                        ...
                        <input class="form-control @error('price') is-invalid @enderror" id="price" name="price" type='number' pattern="^\d{4}*(\.\d{0,2})?$" step="0.01" required min="0.01" max="9999.99" value="{{ empty($dish->id) ? '' : old('price') ?? $dish->price }}">
                        @error('price')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mt-2">
                        ...
                        <input class="form-check-input form-control" id="availability" name="availability" type="checkbox" value="1" {{ $dish->availability ? 'checked' : '' }}>
                    </div>
                    ...
                    <div class="img mt-3">
                        ...
                        {{-- @if(!empty($dish->image)) --}}
                        <img class="img-fluid my-3" src="{{ !empty($dish->image) ? asset('storage/' . $dish->image) : asset('storage/uploads/placeholder.png') }}">
                    ...
                    {{-- Form submit-button --}}
                    <button type="submit" class="btn btn-success mt-5"><i class="fa-solid fa-floppy-disk me-2"></i>Salva</button>
    ...
    {{-- IMAGE DELETION FORM --}}
    @if (!empty($dish->image))
    <form action="{{ route('admin.dish.destroyImage', $dish) }}" class="d-none" method="POST" id="delete-image-form">
        @csrf
        @method('DELETE')
    </form>
    @endif
