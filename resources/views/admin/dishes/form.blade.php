...
@section('content')
    ...
    @endif --}}
            ...
            <a href="{{ route('admin.dishes.index') }}" class="btn btn-primary my-3"><i
                    class="fa-solid fa-table-list me-2"></i>Back to the List</a>
                        ...
                        <div class="card p-2">
                            <div>
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name"
                                    value="{{ empty($dish->id) ? '' : old('name') ?? $dish->name }}" required
                                    max="150" />
                                @error('name')
                            ...
                            <div class="mt-2">
                                ...
                                <input step='0.01' min='0.01' max='9999' type='number'
                                    class="form-control @error('price') is-invalid @enderror" id="price" name="price"
                                    value="{{ empty($dish->id) ? '' : old('price') ?? $dish->price }}" required>
                            ...
                            <div class="mt-2">
                                ...
                                <input type="checkbox" class="form-check-input form-control" name="availability"
                                    id="availability" value="1" {{ $dish->availability ? 'checked' : '' }}>
                            </div>

                            <div class="img mt-3">
                                <label for="image" class="form-label">Image</label>
                                <input type="file" class="form-control @error('image') is-invalid @enderror"
                                    id="image" name="image" value="{{ empty($dish->id) ? '' : old('image') }}" />
                                @error('image')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
